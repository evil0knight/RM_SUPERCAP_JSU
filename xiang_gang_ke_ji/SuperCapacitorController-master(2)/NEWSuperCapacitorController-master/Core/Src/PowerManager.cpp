...
struct TempData
{
    ...
    float outputDuty            = 1.0f;//VB/VA,占空比,
    float baseRefereePower      = DEFAULT_REFEREE_POWER;//当前内部算出来的“基础裁判功率目标”,
                                                        //就是根据能量缓冲加上CAN发过来的数据综合考量之后,电池端输出期望功率
    float targetRefereePower    = DEFAULT_REFEREE_POWER;//真正喂给功率环 pidPowerReferee 的目标值,可能本来还想对baseRefereePower做一些处理
                                                        //但是实际上这个工程没有再进一步的处理了,直接赋值了
    float lastRefereePowerLimit = DEFAULT_REFEREE_POWER;//上一次收到的裁判功率上限，用来检测“比赛给的功率档位是不是变了”
...
{
    /* start HRTIM PWM */
    //MASTER是主中断,TA和TB是受主中断里面东西控制,只会无脑输出PWM
...
{
    this->vASide = CALI_VA(sampleVASide.sum);//换算实际电压值
    this->vASide = M_MAX(this->vASide, 0.4f);//限幅
    ...
    if (this->pASide > this->pBSide)//检查转化效率A侧转B侧(大概充电)
    {
        if (this->pBSide > 1.0f)
    ...
    this->capCharge += this->iBSide;//用于检测,这里测算电荷流量
                                    //后面错误检测里:如果说电荷有流量但是电压一直不变,那么就是电容连接有问题
                                    //也就是电子负载仪不能代替电容的原因
}

/*
*这里__attribute__((section(".BUFFER")))是因为链接器将所有全局变量放到了CCRAM,而这个区域芯片不允许DMA读取
所以这里更改了数组的存储区域,让DMA可以读取
*/
//定义DMA接收数组
...
// [新增] 两路电流动态零点变量，上电校准后覆盖，初始值为编译期拟合值
float iABiasDynamic  = -40.3065f;
float iBBiasDynamic  = 40.8592f;
float iAZeroSumDebug = 0.0f;  // [新增][DEBUG] 供 Ozone 观察
float iBZeroSumDebug = 0.0f;  // [新增][DEBUG]

// [新增] 上电零点校准：buckboost 未工作时 IA/IB 为 0，采样均值反推 BIAS
// iReferee 不参与动态校准（裁判系统上电即有待机电流）
static void calibrateCurrentZeroPoints()
{
    HAL_Delay(50);  // 等待 DMA 稳定

    constexpr int N = 32;
    uint32_t accIA = 0, accIB = 0;
    for (int i = 0; i < N; i++)
    {
        adcIA.sumBuffer();
        adcIB.sumBuffer();
    ...
    float sumZeroIA = (float)accIA / N;
    float sumZeroIB = (float)accIB / N;

    iAZeroSumDebug = sumZeroIA;  // [DEBUG]
    iBZeroSumDebug = sumZeroIB;  // [DEBUG]

    if (sumZeroIA < 15000.0f || sumZeroIA > 18000.0f ||
        sumZeroIB < 15000.0f || sumZeroIB > 18000.0f)
    {
        while (1) {}
    }

    iABiasDynamic = -I_A_CONVERT_GAIN * (sumZeroIA * DIVIDED_BY_ADC_SAMPLE_COUNT);
    iBBiasDynamic = -I_B_CONVERT_GAIN * (sumZeroIB * DIVIDED_BY_ADC_SAMPLE_COUNT);
}
...
// DEBUG: 过压问题调试用，问题解决后删掉
static volatile float debug_deltaVoltage = 0;
static volatile float debug_deltaCurrent = 0;
static volatile float debug_dutyDiffRatio = 1.0f;
static volatile int   debug_voltageLoopActive = 1; // 1=电压环主导 0=电流环主导
...
{
struct ErrorCheckData//这个结构体用来服务错误检测函数
{
    uint16_t restartCoolDown        = 0;//重新启动冷却时间计数
    uint16_t errorCoolDown          = 0;//报错冷却时间计数
    uint16_t shortCircuitCnt        = 0;//短路计数
    float lastCapVoltage            = 0.0f;//上次电容电压
    float capVoltageChange          = 0.0f;//电容电压改变量,这两个都是在错误检测里要用的
    uint16_t noCapCheckCnt          = 0;//没用到
    uint16_t buckBoostCheckCnt      = 0;//buckboost错误检测计数
    uint16_t overVoltageCnt         = 0;//电池和电容过压错误检测计数
    uint8_t currentError            = 0;//当前检测到的错误位集合
    /*0x01 = ERROR_UNDER_VOLTAGE
  - 0x02 = ERROR_OVER_VOLTAGE
  - 0x04 = ERROR_BUCK_BOOST
  - 0x08 = ERROR_SHORT_CIRCUIT
  - 0x10 = ERROR_HIGH_TEMPERATURE
  - 0x20 = ERROR_NO_POWER_INPUT
  - 0x40 = ERROR_CAPACITOR*/

    uint16_t capacitanceEstimateCnt = 0;//和上边两个浮点数是一起的还有一个capCharge电荷量
                                        //,因为是要有测量窗口的不能无限累计
    int16_t capErrorCnt             = 0;//检测电容端接的是不是电容,不是就报错
    uint16_t lowBatteryWarningCnt   = 0;//低电压计数,不过不会报错,只会触发蜂鸣器
} errorCheckData;
//IGNORE_CAPACITOR_ERROR定义了电容相关的检测就用不上
bool checkHardwareError()//就开头调用一次
{
    ...
    for (uint16_t i = 0; i < 10; i++)
        if (isnan(SampleManager::ProcessedData::processedData.pReferee) || isnan(SampleManager::ProcessedData::processedData.iReferee) ||
            ...
            SampleManager::adcVB.sum == 4095U * ADC_SAMPLE_COUNT)//看看浮点数正不正常,超电应该没有充电或者放电,底盘被供电的电流应该小于8A
            //SampleManager这些值在判断之前DMA都打开了,所以现在应该是满量程的1半左右才正常
            while (true)//如果有异常就
            {
                Status::status.errorCode |= ERROR_BUCK_BOOST;//功率板状态不对
                TimerManager::HRTIMDisableOutput();//主中断关闭,不进入升降压
                Buzzer::play(2500, 1000);//蜂鸣器报警
                HAL_Delay(2000);
...
static void handleShortCircuit()//这个是主中断里调用的短路检测
{
    ...
    {//如果电压小于5V
        ...
        {//如果电流大于15.5A
            errorCheckData.currentError |= ERROR_SHORT_CIRCUIT;//短路报错
            Status::status.errorCode |= ERROR_SHORT_CIRCUIT;//短路报错
            if (TimerManager::getHRTIMOutputState() == true)//如果主中断打开了
            {
                if (errorCheckData.shortCircuitCnt++ >= 5)//短路故障累计计数>5
                {
                    errorCheckData.shortCircuitCnt        = 5;
                    ControlData::controlData.enableOutput = false;//禁止输出状态
                }
                TimerManager::HRTIMDisableOutput();//关主中断
                errorCheckData.restartCoolDown = 500;//重启冷却
                errorCheckData.errorCoolDown   = 500;//错误冷却
                Buzzer::play(200, 100);
            }
...
static void handleErrorState()//辅助中断里调用
{
    /* Under Voltage */
#ifdef CALIBRATION_MODE
    if (SampleManager::ProcessedData::processedData.vASide > 0.5f && SampleManager::ProcessedData::processedData.vBSide > 0.5f &&
        SampleManager::ProcessedData::processedData.vASide < 12.0f && SampleManager::ProcessedData::processedData.vBSide < 12.0f)
    {
        ControlData::controlData.enableOutput = false;
...
#else
    if (SampleManager::ProcessedData::processedData.vASide < 15.0f)//A侧无电压报错
    {
        errorCheckData.currentError |= ERROR_NO_POWER_INPUT;//报错无输入电压
        Status::status.errorCode |= ERROR_NO_POWER_INPUT;
        if (SampleManager::ProcessedData::processedData.vBSide < 12.0f)//也无电容报错
        {
            errorCheckData.currentError |= ERROR_UNDER_VOLTAGE;//报错缺少电容
            Status::status.errorCode |= ERROR_UNDER_VOLTAGE;
        }
        else
        {
            errorCheckData.currentError &= ~ERROR_UNDER_VOLTAGE;//又有电容了
            Status::status.errorCode &= ~ERROR_UNDER_VOLTAGE;
        }
        Status::status.lowBattery = false;//标记无电池状态
    }
    else
    {//那就是有电池了
    ...
    {//当电池电压改变大于3V的时候
        //SampleManager::ProcessedData::processedData.capCharge电荷改变量
        //errorCheckData.capVoltageChange电压改变量
    ...
    debug_dutyDiffRatio = dutyDiffRatio;
    if (Status::status.outputEnabled &&
        (((SampleManager::ProcessedData::processedData.pASide > 10.0f || SampleManager::ProcessedData::processedData.pBSide > 10.0f) &&
         ...
         dutyDiffRatio > 3.0f ||
         dutyDiffRatio < 0.25f))  // duty ratio abnormal,这里我改了,本来效率是0.5和2
    {//这里是效率判断
...
powerManagerInit:
    ...
    ControlData::controlData.enableOutput = 1;
    //ControlData::controlData.enableOutput = false;
    ...
    // [新增] 上电零点校准，HRTIM 中断期间暂时屏蔽，防止控制环干扰
    SampleManager::calibrateCurrentZeroPoints();
...
static inline void updateVIP()
{
    ...
    if (Status::status.outputEnabled)  //[[likely]]
    {
        ...
        pidPowerReferee.update(tempData.targetRefereePower, SampleManager::ProcessedData::processedData.pReferee);//更新功率环
        //功率环输出期望功率,然后除个电压就能得到最终的电流
        ...
        float powerALimitNegativeBoundByA = -1 * I_LIMIT * tempVASide;//A端功率限制
        float powerALimitPositiveBoundByA = I_LIMIT * tempVASide;
        //A侧功率 ≈ B侧允许电流 × A侧电压 × (IA / IB)
        float powerALimitNegativeBoundByB = -1 * tempCapOutILimit * tempVASide * actualIAtoIB;//B端的功率限制
        ...
        float powerLimitBToA = M_MAX(powerALimitNegativeBoundByA, powerALimitNegativeBoundByB);//因为这里是负数,还是选最保守的那个
        ...
        tempData.targetIA = tempData.targetAPower / tempVASide;//这里功率环算出来,然后一系列的限幅然后输出功率
                                                    //然后除个电压就是电流

        pidCurrentA.update(tempData.targetIA, SampleManager::ProcessedData::processedData.iASide);

        static float extraErrorSum = 0;//累计误差项
        static float kII           = 1.1e-05f;//累计误差项系数
        ...
        //这里的PWM比例是上次占空比(tempData.outputDuty)+PID修正量+作者在外面加的积分项补丁(kII * extraErrorSum)
        float iaDuty = tempData.outputDuty + (pidCurrentA.getDeltaOutput() + kII * extraErrorSum);
        debug_deltaCurrent = pidCurrentA.getDeltaOutput(); // DEBUG: 问题解决后删掉

        // 原来: 触发阈值 MAX_CAP_VOLTAGE*0.9f (25.92V)，目标 MAX_CAP_VOLTAGE (28.8V)
        // 问题: 误差为正(+2.88V)，电压环一直输出正增量，不但没限压反而加速充电，导致电容反灌电源侧过压
        if (SampleManager::ProcessedData::processedData.vBSide > MAX_CAP_VOLTAGE * 0.9f)
        {//当电容电压超过26.5V的时候
            //电压环接管,目标电压27V，接管时误差为负，电压环输出负增量压制占空比
            pidVoltageB.update(MAX_CAP_VOLTAGE, SampleManager::ProcessedData::processedData.vBSide);//电压环接管电流环
            debug_deltaVoltage = pidVoltageB.getDeltaOutput(); // DEBUG: 问题解决后删掉

            static volatile float vbDuty = 1;
            //以之前算出的outputDuty把这个当做VB/VA,然后乘VA算出的就是VB然后再除VA算出的就是VB/VA就是占空比
            ...
            if (vbDuty < iaDuty)//取保守值
            {
                debug_voltageLoopActive = 1; // 电压环主导
                tempData.outputDuty = vbDuty;
                extraErrorSum       = 0;//全面抛弃IA时期的占空比
                if (tempData.baseRefereePower > ControlData::controlData.refereePowerLimit + 3.0f)//限制功率环输入
                    tempData.baseRefereePower = ControlData::controlData.refereePowerLimit + 3.0f;
                if (tempData.targetAPower > SampleManager::ProcessedData::processedData.iBSide * MAX_CAP_VOLTAGE + 8.0f)//限制电流环输入
                    tempData.targetAPower = SampleManager::ProcessedData::processedData.iBSide * MAX_CAP_VOLTAGE + 8.0f;
            }
            else
            {
                debug_voltageLoopActive = 0; // 电流环主导
...
static inline void updateStatus()//这里是控制LED和CAN发送超电状态的
{
    //B端和A端的电压比
    ...
    Status::status.outputEnabled = TimerManager::getHRTIMOutputState();//超电有没有使能

    Status::status.capEnergy =//超电能量
...
void updateEnergy()
{
    if (tempData.lastRefereePowerLimit != ControlData::controlData.refereePowerLimit)//如果裁判系统的功率变了
    {                                       //refereePowerLimit从CAN里读出来的
        tempData.lastRefereePowerLimit = ControlData::controlData.refereePowerLimit;//赋值新的功率
        tempData.baseRefereePower += ControlData::controlData.refereePowerLimit - tempData.targetRefereePower;
        //他这么写比较绕,实际上就是直接赋值了
        pidPowerReferee.reset();//清理增量式记录,虽然不平滑,但是保证了新值的安全,没有滞后性,反应更快速
    }
    if (ControlData::controlData.energyRemain > ENERGY_BUFFER / 2.0f)//能量缓冲充足
    {
        if (Status::status.communicationTimeoutCnt < 80)//这里调整一下能量缓冲的处理频率
            return;
        PowerManager::Status::status.communicationTimeoutCnt = 0;
        pidEnergyReferee.update(ENERGY_BUFFER, ControlData::controlData.energyRemain);//能量缓冲PID更新一下,能量缓冲期望剩余能量固定为ENERGY_BUFFER
        //这是最外环能量环
        tempData.baseRefereePower -= pidEnergyReferee.getDeltaOutput();//根据减少的能量缓冲用PID转化成功率
        tempData.baseRefereePower = M_CLAMP(tempData.baseRefereePower, 10.0f, ControlData::controlData.refereePowerLimit + 30.0f);//能量环限幅
    }
    else//在能量不充足的情况下,能量环输出固定为10W
    {
        PowerManager::Status::status.communicationTimeoutCnt = 0;
        pidEnergyReferee.updateDataNoOutput(ENERGY_BUFFER, ControlData::controlData.energyRemain);
        tempData.baseRefereePower = 10.0f;//电池端的输出功率固定为10V
    }
 ...
 */
extern "C"
{//主循环没有,是因为这是数控电源,采样的频率需要控制
    void HRTIM1_Master_IRQHandler(void)  // V I loop,这个中断是最重要的中断,主中断
    {
        __HAL_HRTIM_MASTER_CLEAR_IT(&hhrtim1, HRTIM_MASTER_IT_MREP);//清标志位

        float cnt = __HAL_TIM_GET_COUNTER(&htim16);//读取定时器的计数寄存器
        __HAL_TIM_SET_COUNTER(&htim16, 0);

        SampleManager::adcVA.sumBuffer();//加和ADC为了求平均
        ...
        static volatile float load;//没用到
    ...
    void TIM2_IRQHandler(void)
    {
        ...
        updateStatus();//状态上报
        ErrorChecker::handleErrorState();//所有错误检测
